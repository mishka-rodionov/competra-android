...
Статус: **реализовано 23.09.2026 (этапы 0–6)**; остаётся проверка на реальном соревновании и пункты «Открытые вопросы» / этап 7.
...
**Реализовано (этап 3) — уточнения контракта относительно текста выше (приоритет у этого блока):**
- Курсор `live` — **непрозрачная строка** `epoch:seq`, а не число: `seq` — номер точки в порядке добавления в память процесса (не id строки в БД — при параллельных батчах точка с меньшим id может закоммититься позже и проскочить курсор). `epoch` меняется при рестарте процесса.
- Ответ `live`: `{ cursor, reset, serverTime, sessions }`. `sessions` — метаданные **всех** сессий дистанции в памяти (активные и закрытые за последний час) при каждом запросе, `points` — только новые после курсора, `[t, lat, lon]`, в порядке приёма (клиент сортирует по `t`). `reset = true` — курсор не узнан: выбросить накопленное, в ответе полные треки. Первый запрос — без `since`.
- Клиент бегуна отправляет батчи **строго последовательно** (следующий — после ответа на предыдущий): батч с `batchSeq` ≤ принятого подтверждается без записи. Ответ `points`/`stop` — `{ status, closeReason, ackedBatchSeq }`; `status ≠ ACTIVE` → клиент останавливает трекинг.
- Старт: ответ `{ sessionId, status, closeReason, lastBatchSeq, uploadIntervalSec, deadlineAt }`; при возобновлении клиент продолжает нумерацию с `lastBatchSeq + 1`. Карта у дистанции **не обязательна** (зритель без карты видит трек на OSM). Коды: 401 без JWT, 403 чужой участник/сессия, 404 нет участника/сессии, 422 нет согласия / не день соревнования / уже есть результат / у группы нет дистанции, 503 процесс ещё поднимает схему.
- Окно старта: `[startDate − 1 сут; (endDate ?: startDate) + 1 сут]`, статус соревнования не `FINISHED`/`ARCHIVED`. `deadlineAt` = `max(now, плановый старт)` + КВ группы/соревнования + 60 мин, без КВ — + 6 ч.
- Точки: не больше 300 в батче (иначе 400); точки вне `[startedAt − 5 мин; now + 1 мин]` или с координатами вне диапазона молча отбрасываются, батч принимается.
- `result.saved` публикует `OrienteeringResultService.upsert/upsertAll` через `ResultEventPublisher` (свой поток, после коммита, при недоступном брокере события 30 с отбрасываются без попыток). Закрывают сессию только финальные статусы: `FINISHED`, `DSQ`, `DNS`, `DNF`, `OVERTIME`. Очередь `live-track.result-saved` — **без DLX**: необработанное событие отбрасывается, сессию закроет сторож.
- Сторож раз в минуту; закрытые сессии держатся в памяти 1 ч, после рестарта процесса память восстанавливается из БД (активные + закрытые за час).
...
**Реализовано (этап 4):**
- `:core:tracking`: `GpsLocationSource` (общий с трекингом тренировок), `LiveTrackEngine` (старт, запись точки, шаг отправки, стоп; батчи строго по одному под `Mutex`), `LiveTrackRecorderState`, `CompetitionTrackingController`, `LiveTrackFlushWorker` (досылка, когда сервис не работает; ставится на старте приложения и при появлении сети).
- Буфер — таблицы Room `live_track_runner_sessions` / `live_track_runner_points` (миграция 51→52). Назначенный, но не подтверждённый батч (`inFlightBatchSeq`) отправляется повторно с тем же номером. При старте/возобновлении локальная нумерация согласуется с `lastBatchSeq` сервера (переустановка приложения не ломает нумерацию).
- Старт — только `competitionId`: участник ищется сервером по `userId` из JWT (`participantId` необязателен).
- Пустой батч-«пульс» раз в 30 с, если точек нет, — так телефон узнаёт, что сервер закрыл сессию.
- Закрытие сессии сервером (результат/таймаут) останавливает GPS, но буфер досылается: сервер 2 ч после закрытия **дописывает** в трек точки, снятые до момента закрытия (решение 23.09.2026).
- 4xx: 403/404 — сессия потеряна, буфер выбрасывается; прочие 4xx — выбрасывается только битый батч. Сеть/5xx — повтор 10→20→40→80→120 с.
- `CompetitionTrackingService` (`:app`, FGS location): фикс раз в 2 с / 3 м, точность хуже 50 м отбрасывается; после окончания записи досылает не дольше 30 с, дальше — воркер; `START_STICKY`.
- UI (`:core:eventdetails`): кнопка «Включить онлайн-трек» (зарегистрирован, ориентирование, день соревнования ± сутки, не FINISHED/CANCELLED) → однократное согласие → разрешения (FINE_LOCATION, POST_NOTIFICATIONS) → старт → экран `LiveTrackRunnerRoute` без карты: статус, длительность, GPS, передача, очередь; «Остановить трек» с подтверждением, «Продолжить запись», если сервис был убит.
- Аналитика: `live_track_started(competition_id, is_resumed)`, `live_track_stopped(competition_id, reason, duration_min)`, экран `event_live_track_runner`.
...
**Реализовано (этап 5):**
- Кнопка «Онлайн-треки участников» на деталях события (ориентирование; соревнование идёт **или** по нему уже есть треки — один запрос `/competitions/{id}/distances`).
- `LiveTracksRoute` — список дистанций с треками (счётчики «на дистанции / треков», пометка «карта не загружена»), обновление раз в 15 с. `LiveTrackMapRoute` — карта дистанции.
- Карта: osmdroid MAPNIK + `GroundOverlay` растра по 4 углам (`DistanceMap.corners()`, растр через Coil ≤ 4096 px), КП — пурпурные окружности (финиш — двойная), треки цветами участников с разрывами > 30 с, маркер-кружок с номером на последней точке (серый — нет данных > 60 с по `serverTime`, полупрозрачный — закрыт), фильтр групп, «Хвост 5 мин», список участников под картой (тап — центрировать).
- Данные: архив `tracks` один раз + опрос `live` с курсором через `LiveTrackAccumulator` (domain; слияние без дублей по `t`, сортировка, `reset`) — раз в 5 с, пока кто-то на дистанции, иначе раз в 30 с; опрос только пока экран виден (`LifecycleStartEffect`).
- Аналитика: `event_live_tracks_opened(event_id)`, `live_track_map_opened(competition_id, distance_id, mode: live|archive)`, экраны `event_live_tracks`, `event_live_track_map`.
...
**Реализовано (этап 6):** вкладка «Онлайн-треки» на странице соревнования (соревнование идёт или треки есть — `useTrackedDistances`, ошибка трекинга вкладку просто скрывает) → карточки дистанций (обновление 15 с) → `/competition/:id/live-tracks/:distanceId` (`LiveTrackMapPage`): `LiveTrackMapView` поверх `DistanceMapView` (растр по углам через `RotatedImageOverlay`) или чистого OSM (вписывание по КП/трекам один раз), КП — `Circle`, треки — `Polyline` по отрезкам, маркер — `divIcon` с номером; фильтр групп, «Хвост 5 мин», список участников (клик — `flyTo`). Данные: `src/lib/liveTrackAccumulator.ts` (порт domain-накопителя), опрос 5/30/10 с, пауза при скрытой вкладке (`visibilitychange`). Аналитика — те же `event_live_tracks_opened` / `live_track_map_opened`, экран `event_live_track_map`.
...
- Источник OSM-тайлов для продакшна: `tile.openstreetmap.org` по usage policy не рассчитан на нагрузку приложений (касается и osmdroid, и Leaflet). Базовые требования выполнены (24.09.2026): User-Agent = имя пакета (его ставит `Configuration.load()` osmdroid), атрибуция на всех картах Android (`CopyrightOverlay`) и web. До публикации в магазинах/роста аудитории — перейти на провайдера с ключом (MapTiler / Stadia / Thunderforest Outdoors) или свой тайл-сервер.
